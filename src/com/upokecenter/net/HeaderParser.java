...
public final class HeaderParser {
	...
	public static String formatDate(long date){
		...
		if(dow==Calendar.SUNDAY)dayofweek="Sun, ";
		if(dow==Calendar.MONDAY)dayofweek="Mon, ";
		if(dow==Calendar.TUESDAY)dayofweek="Tue, ";
		if(dow==Calendar.WEDNESDAY)dayofweek="Wed, ";
		if(dow==Calendar.THURSDAY)dayofweek="Thu, ";
		if(dow==Calendar.FRIDAY)dayofweek="Fri, ";
		if(dow==Calendar.SATURDAY)dayofweek="Sat, ";
	...
	public static int skipDirective(String str, int io){
		int length=str.length();
		char c=0;
		while(io<length){ // skip non-separator
			c=str.charAt(io);
			if(c=='=' || c==',' || c==127 || c<32)break;
			io++;
		}
	...
	public static int parseTokenWithDelta(String str, int index, String token, int[] result){
		int length=str.length();
		int j=0;
		int startIndex=index;
		result[0]=-1;
	...
	public static int parseToken(String str, int index, String token, boolean optionalQuoted){
		int length=str.length();
		int j=0;
		int startIndex=index;
		for(int i=index;i<length && j<token.length();i++,j++){
	...
	public static String getCharset(String contentType){
		...
		while(true){
			...
			if(c!='C' && c!='c')ischarset=false;
			if(ischarset)c=contentType.charAt(io++);
			if(ischarset && c!='H' && c!='h')ischarset=false;
			if(ischarset)c=contentType.charAt(io++);
			if(ischarset && c!='A' && c!='a')ischarset=false;
			if(ischarset)c=contentType.charAt(io++);
			if(ischarset && c!='R' && c!='r')ischarset=false;
			if(ischarset)c=contentType.charAt(io++);
			if(ischarset && c!='S' && c!='s')ischarset=false;
			if(ischarset)c=contentType.charAt(io++);
			if(ischarset && c!='E' && c!='e')ischarset=false;
			if(ischarset)c=contentType.charAt(io++);
			if(ischarset && c!='T' && c!='t')ischarset=false;
			if(!ischarset){
				io=startio;
				while(io<length){ // skip non-separator
					c=contentType.charAt(io);
